...
class MWOA:
    ...
    def start(self):
        ...
        while t < self.MAX_Iter:
            ...
            prob = 0.01 + 0.01 *(1 - t / self.MAX_Iter) #MWOA
# global
            for i in range(self.popSize):
                q = np.random.random()
            ...
            for i in range(self.popSize):
                ...
                a_fin=0
                g=g-2*g*g
                a=a_init*abs(g)-(a_init-a_fin)*math.tan(0.8*t/self.MAX_Iter)'''
                a = 2 - (2 * t / self.MAX_Iter)#调整算法的行为
                r = np.random.random()
                o=np.random.random()
                A = 2 * a * r - a
                C = 2 * o
                ...
                D_ = [abs( idx1 - idx2) for idx1, idx2 in zip(self.bestPops, self.pops[i])]
                ...
                if p < 0.5:
                    if abs(A) < 1:
                        ...
                        pop_ = [round(idx1 -  A * idx2) for idx1, idx2 in zip(self.bestPops, D)]
                         ...
                         pop_ = [round(idx1 - levy_random_numbers * idx2 ) for idx1,idx2 in zip(self.pops[rand],D__)]
                else:
...
class WOA:
    ...
    def start(self):
        ...
        for newServiceFeature, constraints, minCost, solution, idx in zip(newServiceFeatures, constraintsList,
            ...
            if not solution:
                        model = MWOA(newServiceFeature, constraints, popSize=self.popSize, MAX_Iter=self.MAX_Iter, y='buyuan')
            else:
                        model = MWOA(newServiceFeature, constraints, solution, popSize=self.popSize, MAX_Iter=self.MAX_Iter)
            q, sol = model.start()
