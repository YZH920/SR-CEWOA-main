...
class ASWOA:
    ...
    def calc(self, services): # solution  == services
        ...
        for i in range(len(self.constraints)):#2
    ...
    def start(self):
        ...
        while t < self.MAX_Iter:
            ...
            for i in range(self.popSize):
                ...
                l = np.random.random()
                p = np.random.random()
                D = [abs(C * idx1 - idx2) for idx1, idx2 in zip(self.bestPops, self.pops[i])]
                rand = np.random.randint(0, len(self.pops[i]))
                D__=[abs(C * idx1 - idx2) for idx1, idx2 in zip(self.pops[rand],self.pops[i])]
                pop_ = None
                w=(self.MAX_Iter*self.MAX_Iter*self.MAX_Iter-t*t*t)/(self.MAX_Iter*self.MAX_Iter*self.MAX_Iter)

                if p < 0.5:
                    if abs(A) < 1:
                        #ASWOA
                        pop_ = [round(idx1 - w * A * idx2) for idx1, idx2 in zip(self.bestPops, D)]

                    else:
                         phi = (gamma(1 + 1.5) * np.sin(np.pi * 1.5 / 2) / (gamma((1 + 1.5) / 2) * 1.5 * 2 ** ((1.5 - 1) / 2))) ** (1 / 1.5)
                         mu = np.random.normal(0, 1)
                         nu = np.random.normal(0, 1)
                         levy_random_numbers = phi * mu / (np.abs(nu) ** (1 / 1.5))
                         pop_ = [round(idx1 + levy_random_numbers * idx2 * 0.05) for idx1,idx2 in zip(self.pops[rand],D__)]
                else:
...
class WOA:
    ...
    def start(self):
        ...
        for newServiceFeature, constraints, minCost, solution, idx in zip(newServiceFeatures, constraintsList,
            ...
            t = time.time()
            '''if len(newServiceFeature)==50:#3013 3040 3041 3075
                print(idx)
            continue
            exit()'''
            #if idx==3003:
            if not solution:
                        model = ASWOA(newServiceFeature, constraints, popSize=self.popSize, MAX_Iter=self.MAX_Iter, y='buyuan')
            else:
                        model = ASWOA(newServiceFeature, constraints, solution, popSize=self.popSize, MAX_Iter=self.MAX_Iter)
            q, sol = model.start()
            #exit()
